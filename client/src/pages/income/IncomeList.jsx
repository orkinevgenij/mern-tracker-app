...
const IncomeList = () => {
  ...
  const { incLoading, incAppErr, incServerErr, incomeList } = useSelector((state) => state?.income);
              ...
              <table className='table'>
                ...
                <tbody>
                  {incLoading ? (
                    ...
                    <h1>Доходы отсутствуют</h1>
                  ) : (
                    incomeList?.docs?.map((inc) => <ContentDetails item={inc} key={inc._id} />)
                  )}
                </tbody>
              </table>
            </div>
